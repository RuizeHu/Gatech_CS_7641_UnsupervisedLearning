...
# PCA with DS1
start = time.time()
pca = KernelPCA(n_components = 8, kernel='poly')
pca.fit(x_train_scaled)
end = time.time()
print("KPCA for DS1 time:", end-start)
axs[0].bar(range(1,9), pca.lambdas_)
...
error = []
for i in range(1,9):
    pca = KernelPCA(n_components = i, kernel='poly',fit_inverse_transform=True)
    pca.fit(x_train_scaled)
...
# PCA with DS2
start = time.time()
pca = KernelPCA(n_components = 28, kernel='poly')
pca.fit(x_train_scaled)
end = time.time()
print("PCA for DS2 time:", end-start)
axs[2].bar(range(1,29), pca.lambdas_)
