...
data = ica.fit_transform(x_train_scaled)
end = time.time()
print("ICA for DS1 time:", end-start)
ksort = np.sort(kurtosis(data))
ksort_inv = ksort[::-1]
axs[0].bar(range(1,9), ksort_inv)
axs[0].set_title('DS1 kurtosis')
...
for i in range(1,9):
    ...
    x_train_scaled_ica = ica.fit_transform(x_train_scaled)
    x_reconstructed = ica.inverse_transform(x_train_scaled_ica)
    reconstruct_error = 0
...
# PCA with DS2
start = time.time()
ica = FastICA(n_components=28, max_iter=1000)
data = ica.fit_transform(x_train_scaled)
end = time.time()
print("ICA for DS2 time:", end-start)
ksort = np.sort(kurtosis(data))
ksort_inv = ksort[::-1]
axs[2].bar(range(1,29), ksort_inv)
axs[2].set_title('DS2 kurtosis')
